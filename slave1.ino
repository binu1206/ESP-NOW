...
struct_message_send1 lastReceivedDhtData1;
...
void loop() {
  ...
  if (currentMillis - lastSendTime >= 3000) {
    ...
    lastReceivedDhtData1.lightState1 = digitalRead(RELAY_LIGHT_PIN);
    lastReceivedDhtData1.fanState1 = digitalRead(RELAY_FAN_PIN);
    Serial.println("Sending data...");
    esp_err_t result = esp_now_send(broadcastAddress, (uint8_t *) &lastReceivedDhtData1, sizeof(lastReceivedDhtData1));
  }  
...
if (!isnan(lastReceivedDhtData1.temp1)) {
    if (lastReceivedDhtData1.temp1 > TEMP_THRESHOLD && !fanManualOverride) {
      digitalWrite(RELAY_FAN_PIN, HIGH);
      fanAutoOn = true;
    } else if (lastReceivedDhtData1.temp1 <= TEMP_THRESHOLD && fanAutoOn && !fanManualOverride) {
      digitalWrite(RELAY_FAN_PIN, LOW);
      fanAutoOn = false;
    }
  }
  ...
  if (currentMillis - lastPrintTime >= 5000) {
    ...
    lastReceivedDhtData1.temp1 = dht.readTemperature();  // Read temperature in Celsius
    lastReceivedDhtData1.humidity1 = dht.readHumidity(); // Read humidity

    if (isnan(lastReceivedDhtData1.temp1) || isnan(lastReceivedDhtData1.humidity1)) {
      Serial.println("Failed to read from DHT sensor!");
    } else {
      Serial.print("Temperature: ");
      Serial.print(lastReceivedDhtData1.temp1);
      Serial.print(" °C, Humidity: ");
      Serial.print(lastReceivedDhtData1.humidity1);
      Serial.println(" %");
       // Update struct to hold new data
      // lastReceivedDhtData1.fanState1 = digitalRead(RELAY_FAN_PIN);
      Serial.print("Light State: ");
      Serial.print(lastReceivedDhtData1.lightState1);
      Serial.print(", Fan State: ");
      Serial.println(lastReceivedDhtData1.fanState1);
    }
  } 
}
